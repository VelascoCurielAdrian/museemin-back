  ...
  Mutation: {
    crearUsuario: async (_, { input }, {}) => {
      try {
        ...
        const usuarioExiste = await Usuarios.count({ where: { usuario: input.usuario } });
        if(usuarioExiste > 0) throw MESSAGES.existeUsuario;
        await Usuarios.create({ ...input });
    ...
    authenticarUsuario: async (_, {input}, {}) => {
      try {
        ...
        const { isValid, fields, paths } = validator(input);
        if (!isValid) throw new UserInputError('Input Error', { fields, paths });     
        const usernameAvailability = await Usuarios.findOne({ where: { usuario } });
