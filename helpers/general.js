...
const formatError = (
	message = 'Error no espicifado.',
	name = null,
	fieldsTmp = [],
) => {
	const paths =
		fieldsTmp.count > 0
			? fieldsTmp.reduce((paths = [], el) => [...paths, el.name], [])
			: [name];
...
const objectFilter = (
	props,
	filter = [undefined, null, 'undefined', 'null'],
) => {
	let obj = {};
...
const checkEmptyOrRequired = (fields, inputs) => {
	...
	Object.keys(fields).forEach((field) => {
		if (
			...
			response.fields = [
				...response.fields,
				errorResponse({ name: field, type: 'empty' }),
			];
			...
			inputs[field].forEach((interInputs) => {
				Object.keys(fields[field]['fields']).forEach((interField) => {
					if (
						...
						isEmpty(`${interInputs[interField]}`.trim(), {
...
const validData = (
	...
	if (valueType === 1 && isNaN(value)) {
		return [
			...errors,
			{ name, value, message: MESSAGES_VALID_DATA.invalid, ...extra },
		];
	}
	...
	if (valueType === 1)
		finded = inData
			? data.find(
					(tmp) =>
						parseInt(tmp[prop]) === parseInt(value) &&
	...
	if (!validIn && finded) {
		return [
			...errors,
			{ name, value, message: MESSAGES_VALID_DATA.repeated, ...extra },
		];
	}
	if (validIn && !finded) {
		return [
			...errors,
			{ name, value, message: MESSAGES_VALID_DATA.exist, ...extra },
		];
	}
	return errors;
};
		...
		.reduce((newString = defaultTypes, file) => {
			if (file.indexOf('.') !== 0 && file !== 'index.js') {
				const pathFile = path.resolve(
					`${__dirname}/../controllers/${file}/types.js`,
				);
				const tmp = fs.existsSync(pathFile)
					? require(`./../controllers/${file}/types.js`)
					: '';
